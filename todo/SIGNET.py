...
parser.add_argument('--n_genes', type=int, default=200, help='Number of feature genes for training SIGNET')
...
parser.add_argument('--output_path', type=str, default="./signet-outputs", help='The output file path.')
...
parser.add_argument('--tf_list_file', type=str, default="tf_mm.txt",
                    help='The input transcription factor genes file used for prediction.')
parser.add_argument('--use_transpose', type=bool, default=False, help='Whether to transpose the data file or not.')
...
if __name__ == "__main__":
    ...
    opt.data_file = "data/GEO/GSE122662/data-for-the-expert/15kGenes_D0_ipsc_not_norm.csv"
    opt.tf_list_file = "data/mouse_riken_tfs.txt"

    data = sc.read_csv(opt.data_file)
    print(f"dat shape: {data.shape}")
    print(data.X[:2])
    if opt.use_transpose:
        data = data.transpose()

    tf_gene = pd.read_table(opt.tf_list_file, header=None)
    print("data loaded!", "this took:", round(time.time() - start), "seconds.")

    sc.pp.filter_genes(data, min_cells=10)
    data.var["mito"] = data.var_names.str.startswith("")  # TODO: i dunno what 'mito' is
    qc = sc.pp.calculate_qc_metrics(data, qc_vars=['mito'])
    cell_qc_dataframe, gene_qc_dataframe = qc
    ...
    data = data[data.obs.n_genes < 90000, :]  # TODO error here!
    data = data[data.obs.percent_mito < 500, :]  # TODO check this line and the one above
    ...
    assert list3, print("no common genes between tf_gene and raw_data_fe")
    ...
    for i in tqdm(range(raw_data_ntf.shape[0])[:]):
