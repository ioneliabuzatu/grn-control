...
def control(env, num_episodes, num_states, num_genes, num_master_genes):
    start = time.time()
    classifier = None  # TODO used in the cost function, is data-driven
    actions = jnp.zeros(shape=(num_states, num_master_genes)) + 0.1
...
if __name__ == "__main__":
    ...
    if is_debugger_active():
        with jax.disable_jit():
            control(sim, 100, 9, 100, num_master_genes=len(sim.layers[0]))
    else:
        control(sim, 100, 9, 100, len(sim.layers[0]))
